-- Recommendation revamp: dynamic seeded sampling, collaborative signals, dedupe,
-- interaction telemetry, and semantic word-of-mouth. All DDL is idempotent.

-- ============================================================
-- 1. Dedupe columns + word_of_mouth embeddings
-- ============================================================
...
-- ============================================================
-- 2. Interaction log (impressions / taps / remixes / skips)
--    trend_key deliberately has no FK: chatter keys (VIRA-WOM-*) share this log.
-- ============================================================
...
-- ============================================================
-- 3. recommend_company_trends_v2 — wide pool, balanced percentile scoring,
--    live freshness, seeded weighted sampling (Efraimidis-Spirakis).
--    Same seed => same rows (cacheable); new seed => new weighted draw.
-- ============================================================
...
AS $$
  ...
  scored AS (
    SELECT p.*,
      -- Percentile-normalize both signals within the pool so neither dominates
      -- (raw cosine similarities cluster ~0.6-0.85 and would otherwise be near-constant).
      percent_rank() OVER (ORDER BY p.similarity) AS sim_pr,
      -- Live virality: same formula as scoreTrend() in src/lib/trends.server.ts,
      -- with freshness recomputed from posted_at at query time.
      ...
      -- Deterministic uniform noise in (0,1] per (trend, seed).
  ...
  FROM scored s
  -- Efraimidis-Spirakis weighted sampling: top-k by u^(1/w). The floor keeps
  -- low-weight rows in the lottery instead of freezing the top of the list.
...
-- ============================================================
-- 4. Collaborative layer: what similar brands remixed/tapped.
--    SECURITY INVOKER but service_role-only: it must read other owners'
--    interaction rows, so it is only callable through the admin client.
-- ============================================================
...
AS $$
  ...
  peers AS (
    SELECT company_id, MAX(csim) AS csim FROM (
      SELECT company_id, csim FROM similar
      UNION ALL
  ...
  signals AS (
    ...
    UNION ALL
    -- Legacy co-occurrence: remixes that predate the interaction log.
...
-- ============================================================
-- 5. Social proof: distinct-brand remix/tap counts per key.
--    Service_role-only for the same cross-owner reason.
-- ============================================================
...
-- ============================================================
-- 6. Near-duplicate flagging: greedy 8-NN pass; the highest trend_score in a
--    cluster stays canonical. Idempotent, re-runnable, reversible.
-- ============================================================
...
BEGIN
  ...
  -- Collapse chains: repoint duplicates whose canonical got flagged itself.
  FOR i IN 1..3 LOOP
...
-- ============================================================
-- 7. Semantic word-of-mouth v2 (buzz + similarity percentiles, seeded sampling)
--    + lock down the old category-join RPC to match the rest.
-- ============================================================
...
-- ============================================================
-- 8. Keep the category browse path free of flagged duplicates.
-- ============================================================

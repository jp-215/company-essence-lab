-- Terac — expose ad_videos.playback_url to the judge portal.
--
-- terac_open_session() selected playback_id but never playback_url, so the
-- rendered MP4 URLs written by vira-engine could not reach the portal. This
-- redefines the function with one extra key in each videos[] element —
-- 'playback_url' — and changes nothing else: same SECURITY DEFINER posture,
-- same grants, same per-judge video_order, same own-ballot-only join.

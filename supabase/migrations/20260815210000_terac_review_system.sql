-- ============================================================================
-- Terac — application layer for the review schema.
--
-- IMPORTANT: review_sessions, ad_videos, judges, session_judges, video_votes,
-- video_comments and feedback_syntheses ALREADY EXIST in this database. They
-- were created outside the migration history (they appear in
-- src/integrations/supabase/types.ts but no migration creates them).
--
-- This migration therefore CREATES NOTHING THAT EXISTS and DROPS NOTHING.
-- It is additive and idempotent:
--   * adds the few columns the app layer needs
--   * adds public.terac_email_log (the only genuinely new table)
--   * adds the SECURITY DEFINER functions that make the unauthenticated judge
--     portal possible — the live schema has no terac_* functions at all
--
-- Safe to re-run.
--
-- SECURITY MODEL
--   The judge portal is unauthenticated and this project has no service-role
--   key in the server runtime (.env has the publishable key only). So judge
--   access is NOT granted by loosening RLS. Instead anon receives no table
--   grants, and every portal read/write goes through a SECURITY DEFINER
--   function that takes the invite token and resolves its own scope.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- Columns the app layer needs. All IF NOT EXISTS.
-- ---------------------------------------------------------------------------
...
-- Uniqueness the upsert paths rely on. Created only if absent so an existing
-- equivalent index is left alone.
...
-- ---------------------------------------------------------------------------
-- terac_email_log — the one new table. Not in the original spec; added so
-- invites and reminders are inspectable and assertable when no email provider
-- is configured (TERAC_MAIL_DRIVER=log, the default).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Belt and braces: anon must not read these tables directly. Revoking is safe
-- even if it was never granted.
-- ---------------------------------------------------------------------------
...
-- ============================================================================
-- Lifecycle
--   generating -> ready -> sent -> in_review -> complete
--              -> synthesized -> actioned -> closed
-- ============================================================================
...
-- Completes on quorum OR deadline, whichever comes first, so one slow judge
-- cannot block a founder indefinitely.
...
-- NOTE: nothing schedules this. Needs pg_cron or an external ping:
...
-- ============================================================================
-- Judge portal — SECURITY DEFINER, anon-callable, token-scoped.
-- ============================================================================
...
-- Landing + review payload. Marks the invite opened on first call.
--
-- Uses the existing session_judges.video_order column to persist a per-judge
-- shuffle on first open, so order-randomisation survives a reopen instead of
-- reshuffling mid-review. Vote counts are never returned, and the only ballot
-- data in the payload is this judge's own.
      ...
      SELECT jsonb_agg(ordered.payload ORDER BY ordered.position)
        FROM (
          SELECT jsonb_build_object(
                   ...
                   'version', av.version,
                   -- storyboard fallback for judges when no render exists yet
...
-- Autosave one video's ballot. Rejected once the judge has submitted.
...
-- Final submit: writes the drag-ordered ranks, locks the ballot, checks quorum.
-- _ranks is [{"video_id": uuid, "rank": int}, ...].
...
-- Ad-hoc reviewer: someone hits the bare session token with no judge token.
